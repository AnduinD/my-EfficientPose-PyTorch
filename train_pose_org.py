...
class ModelWithLoss(nn.Module):
    def __init__(self, model, param_generator, debug=False, num_gpus=0):
        super().__init__()
        self.criterion_focal = FocalLoss(alpha = 0.25, gamma = 1.5)
        from loss_6DoF import  my_smooth_l1
        self.criterion_l1 = my_smooth_l1(sigma = 3.0) #SmoothL1Loss(beta=3.0)
...
def main(args = None):
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=25, factor = 0.5, verbose=True, eps=1e-10)
    ...
    try:
        for epoch in range(args.epochs):
            ...
            if epoch % val_interval == 0 & epoch > 0:
                model.eval()
