...
class Generator(Dataset):
    ...
    def __init__(
        ...
        self.groups = None
        self.anchor_parameters = AnchorParameters.default #type:ignore
        ...
        if self.use_colorspace_augmentation:
            self.rand_aug = RandAugment(n = (1, 3), m = (1, 14)) # type: ignore
        else:
            self.rand_aug = None
        ...
        if self.shuffle_groups:
            random.shuffle(self.groups) # type: ignore
            
        self.all_3d_model_points_array_for_loss = self.create_all_3d_model_points_array_for_loss(self.class_to_model_3d_points, self.points_for_shape_match_loss) # type: ignore
    ...
    def __getitem__(self, index):
        ...
        """
        index = index % len(self.groups) # type: ignore
        group = self.groups[index] # type: ignore
        inputs, targets = self.compute_inputs_targets(group) # type: ignore
    ...
    def load_mask_group(self, group):
        ...
        """
        return [self.load_mask(image_index) for image_index in group] # type: ignore
    ...
    def load_camera_matrix_group(self, group):
        ...
        """
        return [self.load_camera_matrix(image_index) for image_index in group] # type: ignore
    ...
    def compute_targets(self, image_group, annotations_group):
        ...
        batches_targets = anchor_targets_bbox(
            ...
            num_translation_parameters = self.translation_parameter, #x,y in 2D and Tz # type: ignore
    ...
    def random_transform_group_entry(self, image, annotations, mask, camera_matrix, transform = None):
        ...
        if chance_no_augmentation >= self.chance_no_augmentation:
            ...
            if self.use_colorspace_augmentation:
                ...
                image = self.rand_aug(images = image) # type: ignore
                #squeeze dims again and flip back to BGR
                image = np.squeeze(image)[:, :, ::-1] # type: ignore
    ...
    def augment_6DoF_image_and_annotations(self, img, annotations, mask, camera_matrix):
        ...
        for i in range(num_annos):
            ...
            mask_values[i] = self.name_to_mask_value[self.class_to_name[annotations["labels"][i]]] # type: ignore
        ...
        if is_valid_augmentation:
            for i in range(num_annos):
                annotations["bboxes"][i, :] = augmented_bbox[i, :] # type: ignore
                annotations["rotations"][i, :3] = augmented_rotation_vector[i, :] # type: ignore
                annotations["translations"][i, :] = augmented_translation_vector[i, :] # type: ignore
                annotations["translations_x_y_2D"][i, :] = self.project_points_3D_to_2D(
                                                            points_3D = np.zeros(shape = (1, 3)), #transform the object coordinate system origin point which is the centerpoint
                                                            rotation_vector = augmented_rotation_vector[i, :],  # type: ignore
                                                            translation_vector = augmented_translation_vector[i, :],# type: ignore
                                                            camera_matrix = camera_matrix) 
    ...
    def init_num_rotation_parameters(self, **kwargs):
        ...
        if rotation_representation in self.possible_rotation_representations:
            ...
            self.rotation_parameter = self.possible_rotation_representations[self.rotation_representation] # type: ignore
        else:
    ...
    def group_images(self):
        ...
        self.groups = [[order[x % len(order)] for x in range(i, i + self.batch_size)] for i in #type:ignore
                       range(0, len(order), self.batch_size)] #type:ignore
    ...
    def __len__(self):
        ...
        """
        return len(self.groups) # type: ignore
    ...
    def get_anchors(self):
        ...
        """
        with open(self.anchors_path) as f: # type: ignore
            anchors = f.readline()
