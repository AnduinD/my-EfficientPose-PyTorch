...
class MultiHeadAttention(nn.Module):
    def __init__(self, hidden_size, dropout_rate = None, head_size=8):
        ...
        self.att_dropout = nn.Dropout(dropout_rate) # type: ignore
    ...
    def forward(self, q, k, v, mask=None, cache=None):
        ...
        # 输入的Q、K、V都是[B,C,H,W]顺序的，需要先转换成[B,H*W,C]顺序
        q = q.permute(0,2,3,1).contiguous().view(batch_size, -1, self.hidden_size) # [B, H*W, C(hidden_size)]
        k = k.permute(0,2,3,1).contiguous().view(batch_size, -1, self.hidden_size)
        v = v.permute(0,2,3,1).contiguous().view(batch_size, -1, self.hidden_size)

        # head_i = Attention(Q(W^Q)_i, K(W^K)_i, V(W^V)_i)
        q = self.linear_q(q).view(batch_size, -1, self.head_size, d_k) # [B, H*W, head_size, att_size]
